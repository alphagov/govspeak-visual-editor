...
function parseTable(row, headerRows, cellRows) {
...
function writeCell(state, cell) {
  const paragraph = cell.content.content[0];
...
const tableSerializer = function () {
  return (state, node) => {
    ...
    let cellRows = []
    rows.forEach((row) => {
      parseTable(row, headerRows, cellRows);
    });

    if (headerRows.length > 0) {
      headerRows.forEach((row) => {
        state.write("| ");
        row.forEach((cell) => writeCell(state, cell));
        state.write("\n");
      });
    ...
    cellRows.forEach((cellRow) => {
      cellRow.isColumnHeader ? state.write("# ") : state.write("| ");
      cellRow.row.forEach((cell) => writeCell(state, cell));
      state.write("\n");
    });
