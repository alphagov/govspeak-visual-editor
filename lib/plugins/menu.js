...
function textBlockMenuSelect(schema, editorView) {
  const select = document.createElement("select");
  const textBlockCommands = [
    () => {},
    wrapIn(schema.nodes.call_to_action),
    setBlockType(schema.nodes.information_callout),
    setBlockType(schema.nodes.warning_callout),
    wrapIn(schema.nodes.example_callout),
    wrapIn(schema.nodes.contact),
    wrapIn(schema.nodes.address),
    wrapIn(schema.nodes.blockquote),
  ]
  select.className = "govuk-select test";
  [ "Add text block",
    "Call to action",
    "Information callout",
    "Warning callout",
  ...
  select.addEventListener('change', () => {
    chainCommands(
      textBlockCommands[Number(select.value)],
      setBlockType(schema.nodes.paragraph)
    )(editorView.state, editorView.dispatch, editorView)
    editorView.focus();
    select.selectedIndex = 0;
  ...
  return {
    command: chainCommands(wrapIn(schema.nodes.call_to_action), setBlockType(schema.nodes.information_callout)),
    dom: select,
    customHandler: () => {},
  }
}
...
function items(schema, editorView) {
  return [
    ...
    textBlockMenuSelect(schema, editorView),
    undoMenuItem(schema),
    redoMenuItem(schema),
  ];
}
