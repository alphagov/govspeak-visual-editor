...
const positionIsAtTheEndAfterABreak = (head, schema) => {
  ...
  return head.nodeBefore.type === schema.nodes.hard_break &&
...
const newlineInAddress = (schema, attributes) => {
  return (state, dispatch) => {
    ...
    if (attributes.exitEndOfBlock && positionIsAtTheEndAfterABreak($head, schema)) {
  ...
  return (state, dispatch) => {
    ...
    if (!$head.nodeBefore && !$head.nodeAfter) {
      ...
      dispatch(transaction
        ...
        .setSelection(Selection.near(transaction.doc.resolve(position - 3)), 1)
        .scrollIntoView()
      );
      return true;
    }
