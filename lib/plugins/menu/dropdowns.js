...
import { dispatchSelectCustomEvent } from "./custom-events";
import {
  addColumnAfter,
  addColumnBefore,
  addRowAfter,
...
function selectDom(options, editorView, className, resetOnChange) {
  ...
  select.addEventListener("change", () => {
    ...
    editorView.focus();
    dispatchSelectCustomEvent(select);
...
function tableMenuSelectOptions(schema) {
  return [
    {
      text: "Tables",
      command: () => {},
    ...
    {
      text: "New table",
      command: wrapIn(schema.nodes.table), // Temporary command to get appropriate enabled/disabled behaviour
    },
    {
      text: "Insert column after",
      command: addColumnAfter, // Temporary command to get appropriate enabled/disabled behaviour
    },
    {
    ...
    {
      text: "Align Cells Left",
      command: setCellAttr("horizontalAlign", "left"),
    },
    {
      text: "Align Cells Right",
      command: setCellAttr("horizontalAlign", "right"),
    },
    {
      text: "Align Cells Center",
      command: setCellAttr("horizontalAlign", "center"),
    },
    {
      text: "Set Cells Grey",
      command: setCellAttr("background", "#eeeeee"),
    },
  ];
}
...
export const tableDropdown = (schema, editorView) => {
  return menuSelect(
    tableMenuSelectOptions(schema),
    editorView,
    "visual-editor__menu-table-select",
  );
};
